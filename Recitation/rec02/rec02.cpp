...
struct Hydrocarbon {
    Hydrocarbon(int numofC, int numofH)
        : C(numofC), H(numofH) {}  // constructor
...
void fillHCVector(ifstream& HCStream, vector<Hydrocarbon>& hydrocarbons) {
    ...
    while (HCStream >> name >> skip >> C >> skip >> H) {
        // try to add name of substance to an existing Hydrocarbon data
        // structure. If no existing Hydrocarbon has the required number of H
        // and C, construct a new one.
    ...
    for (size_t i = 1; i < hydrocarbons.size(); i++) {
        for (size_t j = i; j > 0; j--) {
            if (hydrocarbons[j].getNumofC() < hydrocarbons[j - 1].getNumofC()) {
                ...
                if (hydrocarbons[j].getNumofH() <
                    hydrocarbons[j - 1].getNumofH()) {
                    swapHydrocarbons(hydrocarbons, j, j - 1);
                } else {
                    break;
                }
